...
module.exports = async (req, res) => {
  ...
  try {
    ...
    for (const usdIdx of usdIndices) {
      // Take only the first 10 numbers after "USD" — that's exactly one table
      // row's worth of columns, so this can't accidentally spill into the
      // next currency's row even if newline detection is unreliable.
      ...
      if (plausible.length > 0) {
        ...
        result.raw_snippet = window.slice(0, 150);
        break;
      }
    }
